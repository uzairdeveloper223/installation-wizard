...
static int setup(void **state)
{
    ...
    unlink(CONFIG_INSTALL_LOG_PATH);
    callback_count = 0;
...
static void setup_minimal_config(void)
{
    ...
    strncpy(store->locale, "en_US.UTF-8", MAX_LOCALE_LEN);
...
static void test_run_install_sends_step_begin_events(void **state)
{
    ...
    // Should have at least: START, 7x(BEGIN+OK), AWAIT_REBOOT.
    assert_true(callback_count >= 16);
    ...
    assert_int_equal(7, begin_count);
}
...
static void test_run_install_initializes_log(void **state)
{
    ...
    assert_int_equal(0, access(CONFIG_INSTALL_LOG_PATH, F_OK));
}
...
static void test_run_install_writes_log_headers(void **state)
{
    ...
    // Read install log.
    FILE *f = fopen(CONFIG_INSTALL_LOG_PATH, "r");
    assert_non_null(f);
